...
const cartIconQty = document.querySelector(".cart-icon-item-qty");
...
function handleAddToCart() {
  if (itemQty === 0) {
    ...
    updateCartUI();
    cartIconQty.style.display = "flex";
...
function handleQtyChange(incOrDec) {
  if (incOrDec === "dec") {
    ...
    cartIconQty.textContent = `${itemQty}`;
    showItemQty();
  }
  if (incOrDec === "inc") {
    itemQty++;
...
function removeItemFromCart(id) {
  orderItems.forEach((item, idx) => {
    if (item.productId === parseInt(id)) {
      ...
      updateCartUI();
      showItemQty();
      cartIconQty.style.display = "none";
    }
  });
}
